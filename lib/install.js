...
var globalInstall = function (args) {
    ...
    try {
        ...
        var moduleRealPath = '/usr/local/lib/fib_modules/.modules/' + modulename + '/';
        var packageInfo = packaging.analyze(moduleRealPath + '/');
        var bin = packageInfo.bin;
        for (var b in bin) {
            if (fs.exists('/usr/local/bin/' + b)) {
                fs.unlink('/usr/local/bin/' + b);
            }
            process.system('ln -s ' + moduleRealPath + bin[b] + ' /usr/local/bin/' + b);
        }
    } catch (e) {
        console.log((e + '').red.bold + '\nPermission denied --please use sudo'.red.bold);
        return;
    }
