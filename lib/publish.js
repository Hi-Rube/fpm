...
var submit = function (moduleInfo) {
    var protocpl = 'http://';
    var address = (config['registryHost'] ? config['registryHost'] : config['fpmHost']).split(/[http|https]?:\/\//);
    var findPort = address[address.length - 1].split(':');
    if (findPort.length === 2) {
        address[1] = findPort[1];
        address[0] = protocpl + findPort[0];
    } else {
        address[1] = 80;
        address[0] = protocpl + address[0]
    }
