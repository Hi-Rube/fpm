...
var submit = function (moduleInfo) {
  ...
  request.address = '/publish/' + moduleInfo.name;
  request.method = 'POST';
  request.setHeader('Content-Type', 'multipart/form-data; boundary=' + boundary);
  request.setHeader('Content-Length', formStr.length + datas.length + formEnd.length);
  request.write(formStr);
  request.write(datas);
  request.write(formEnd);
  var backInfo = http.request(address[0], address[1], request);
